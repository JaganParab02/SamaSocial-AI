...
**Q: Why does the course planner sometimes hang on generation?**
A: Ensure your `GROQ_API_KEY` is valid and you haven't hit rate limits. Check the backend terminal for specific rate limit exceptions.

**Q: Can I use a local LLM instead of Groq?**
A: Yes! The system is highly modular. You can replace the Groq SDK initialization in `LLMClient` with a local Ollama endpoint or LiteLLM integration.

*For further support or bug reports, please open an issue on the GitHub repository.*
